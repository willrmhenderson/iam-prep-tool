...
function dayBtn(val, label){
  var active = ST.preq.dayRating === val;
  return '<button type="button" class="tbtn' + (active ? " act" : "") + '" aria-pressed="' + active + '" onclick="IAM.setPreq(\'dayRating\',\'' + val + '\')">' + esc(label) + '</button>';
}
function trajBtn(val, label){
  var active = ST.preq.trajectory === val;
  return '<button type="button" class="tbtn' + (active ? " act" : "") + '" aria-pressed="' + active + '" onclick="IAM.setPreq(\'trajectory\',\'' + val + '\')">' + esc(label) + '</button>';
}
function commBtn(val, label){
  var active = ST.preq.commPref === val;
  return '<button type="button" class="tbtn' + (active ? " act" : "") + '" aria-pressed="' + active + '" onclick="IAM.setPreq(\'commPref\',\'' + val + '\')">' + esc(label) + '</button>';
}
...
export function rPreq(){
  ...
  return sb() +
    ...
    '<div class="trow" role="group" aria-labelledby="dayRatingLbl" style="margin-bottom:10px">' +
      dayBtn("good", "Good day") + dayBtn("typical", "Typical day") + dayBtn("hard", "Hard day") +
    '</div>' +
    ...
    '<textarea id="q-daynote" oninput="IAM.setPreq(\'dayNote\',this.value)" placeholder="Record response or note: nothing significant reported">' + esc(q.dayNote) + '</textarea>' +
    '</div>' +
    ...
    '<textarea id="q-disdesc" oninput="IAM.setPreq(\'disabilityDesc\',this.value)" placeholder="Record participant&rsquo;s response">' + esc(q.disabilityDesc) + '</textarea>' +
    ...
    '<div class="trow" role="group" aria-labelledby="trajLbl" style="margin-bottom:10px">' +
      trajBtn("stable", "Stable") + trajBtn("improving", "Improving") + trajBtn("worsening", "Getting worse") +
    '</div>' +
    '<label for="q-change">Optional: Anything significantly better or worse than 12 months ago?</label>' +
    '<textarea id="q-change" oninput="IAM.setPreq(\'changeNote\',this.value)" placeholder="Record if raised">' + esc(q.changeNote) + '</textarea>' +
    '<label for="q-variation">Are there things that change day to day?</label>' +
    '<textarea id="q-variation" oninput="IAM.setPreq(\'dayVariation\',this.value)" placeholder="Record response">' + esc(q.dayVariation) + '</textarea>' +
    '</div>' +
    ...
    '<textarea id="q-history" oninput="IAM.setPreq(\'assessHistory\',this.value)" placeholder="Record response">' + esc(q.assessHistory) + '</textarea>' +
    '<label for="q-difficulty">Was there anything that made it hard to describe yourself accurately in past assessments?</label>' +
    '<textarea id="q-difficulty" oninput="IAM.setPreq(\'assessDifficulty\',this.value)" placeholder="Record response">' + esc(q.assessDifficulty) + '</textarea>' +
    '</div>' +
    ...
    '<div class="trow" role="group" aria-labelledby="commLbl" style="margin-bottom:10px">' +
      commBtn("talk", "Talk through") + commBtn("read", "Read to me") + commBtn("mix", "Mix of both") +
    '</div>' +
    '<label for="q-combarrier">Is anything making it harder to concentrate or communicate today? (pain, fatigue, noise)</label>' +
    '<textarea id="q-combarrier" oninput="IAM.setPreq(\'commBarrier\',this.value)" placeholder="Record response or: nothing reported">' + esc(q.commBarrier) + '</textarea>' +
    '<label for="q-comadjust">Do you need any breaks, a different pace, or adjustments today?</label>' +
    '<textarea id="q-comadjust" oninput="IAM.setPreq(\'commAdjust\',this.value)" placeholder="Record agreed adjustments">' + esc(q.commAdjust) + '</textarea>' +
    '</div>' +
    ...
    '<textarea id="q-sessionflag" oninput="IAM.setPreq(\'sessionFlag\',this.value)" placeholder="Record anything raised">' + esc(q.sessionFlag) + '</textarea>' +
    '</div>' +
    '<div class="nav"><button type="button" class="btn" onclick="IAM.go(\'part\')">&larr; Back</button>' +
    '<button type="button" class="btn primary" onclick="IAM.go(\'brate\')">Next: Before ratings &rarr;</button></div>';
}
