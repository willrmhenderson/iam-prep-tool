...
export function rDomain(i){
  ...
  var dc = "";
  var idp = "dom" + i + "-";
  function domField(key){ return 'data-field="setDom" data-args="' + dataArgs([i, key]) + '"'; }
  function domSelect(key){ return 'data-onchange="setDom" data-args="' + dataArgs([i, key]) + '"'; }

  if (isP || isPsy){
    var goodActive = tab === "good", hardActive = tab === "bad";
    dc += '<div class="trow" role="group" aria-label="Day type">' +
      '<button type="button" class="tbtn' + (goodActive ? " act" : "") + '" aria-pressed="' + goodActive + '" data-action="setDomTab" data-args="' + dataArgs([i, "good"]) + '">Good day</button>' +
      '<button type="button" class="tbtn' + (hardActive ? " act" : "") + '" aria-pressed="' + hardActive + '" data-action="setDomTab" data-args="' + dataArgs([i, "bad"]) + '">Hard day</button></div>';
    if (tab === "good"){
      ...
      dc += '<button type="button" class="btn sm" aria-expanded="' + exOpen + '" data-action="toggleExamples" data-args="' + dataArgs([i]) + '" style="margin-bottom:8px">' +
      ...
      if (exOpen){
        dc += '<div class="exb"><p style="font-size:11px;font-weight:600;color:#8a9e82;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px">Example statements - select to use</p>' +
          d.ex.map(function(ex, ei){ return '<div class="exi"><p>&ldquo;' + esc(ex) + '&rdquo;</p><button type="button" class="btn sm" data-action="useEx" data-args="' + dataArgs([i, ei]) + '">Use this</button></div>'; }).join("") + '</div>';
      }
      dc += '<label for="' + idp + 'gs">' + (isP ? "I can... (on a good day)" : "Good day statement") + '</label>' +
        '<textarea id="' + idp + 'gs" ' + domField("gs") + ' placeholder="' + (isP ? "Start with I can..." : "Enter the participant&rsquo;s good day statement.") + '">' + esc(s.gs) + '</textarea>';
    } else {
      ...
      dc += '<label for="' + idp + 'bs">' + (isP ? "On a hard day, I..." : "Hard day - what changes") + '</label>' +
        '<textarea id="' + idp + 'bs" ' + domField("bs") + ' placeholder="' + (isP ? "e.g. On a hard day I cannot get out of bed without full physical assistance." : "Describe what changes on a hard day.") + '">' + esc(s.bs) + '</textarea>';
    }
    dc += '<div class="g2" style="margin-top:10px">' +
      '<div><label for="' + idp + 'freq">How often is support needed?</label><select id="' + idp + 'freq" ' + domSelect("freq") + '>' + selO(FREQ, s.freq) + '</select></div>' +
      '<div><label for="' + idp + 'stype">Type of support</label><select id="' + idp + 'stype" ' + domSelect("stype") + '>' + selO(STYP, s.stype) + '</select></div></div>' +
      '<label for="' + idp + 'impact">Impact if support not provided</label><select id="' + idp + 'impact" ' + domSelect("impact") + '>' + selO(IMP, s.impact) + '</select>' +
      '<label for="' + idp + 'change">Has this need changed over time?</label>' +
      '<textarea id="' + idp + 'change" ' + domField("change") + ' style="min-height:56px" placeholder="Describe any worsening or progression.">' + esc(s.change) + '</textarea>' +
      '<label for="' + idp + 'notes">Anything else the assessor should know?</label>' +
      '<textarea id="' + idp + 'notes" ' + domField("notes") + ' style="min-height:56px" placeholder="Equipment, who helps, context.">' + esc(s.notes) + '</textarea>';
  }
  if (isS){
    dc += '<div class="tip"><p>Describe what you observe in this area. Be specific.</p></div>' +
      '<label for="' + idp + 'so">What do you observe?</label>' +
      '<textarea id="' + idp + 'so" ' + domField("so") + ' placeholder="Describe what you see day to day.">' + esc(s.so) + '</textarea>' +
      '<div style="margin-top:10px"><label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin:0">' +
      '<input type="checkbox" id="' + idp + 'sf" ' + (s.sf ? "checked" : "") + ' data-onchange="setDomFlag" data-args="' + dataArgs([i]) + '" style="width:auto;margin:0">' +
      ...
      (s.sf ? '<div class="flag"><span style="color:#d4900a;font-size:18px;flex-shrink:0" aria-hidden="true">&#9873;</span>' +
        ...
        '<textarea id="' + idp + 'sfn" ' + domField("sfn") + ' style="background:transparent" placeholder="Explain how your observation differs.">' + esc(s.sfn) + '</textarea></div></div>' : "");
  }
  if (isPsy){
    dc += '<hr style="border:none;border-top:1px solid #eef2ec;margin:1rem 0">' +
      ...
      '<textarea id="' + idp + 'pn" ' + domField("pn") + ' style="min-height:56px" placeholder="Clinical observations or questions to explore.">' + esc(s.pn) + '</textarea>' +
      '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:8px">' +
      '<input type="checkbox" ' + (s.pf ? "checked" : "") + ' data-onchange="setDomPFlag" data-args="' + dataArgs([i]) + '" style="width:auto;margin:0">' +
  ...
  return sb() +
    ...
    dc + '</div>' +
    '<div class="nav">' +
    (i > 0 ? '<button type="button" class="btn" data-action="gd" data-args="' + dataArgs([i - 1]) + '">&larr; Back</button>' :
             '<button type="button" class="btn" data-action="go" data-args="' + dataArgs(["part"]) + '">&larr; Back</button>') +
    '<button type="button" class="btn primary" data-action="saveDom" data-args="' + dataArgs([i]) + '">Save and continue &rarr;</button>' +
    '<button type="button" class="skip" data-action="skipDom" data-args="' + dataArgs([i]) + '">Skip</button></div>';
}
