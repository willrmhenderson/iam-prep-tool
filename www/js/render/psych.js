...
export function rPsych(){
  ...
  return sb() + '<div style="margin-bottom:1rem"><h2 id="scr-h">Psychologist section</h2></div>' +
    ...
    (flagged.length ? '<div class="sec" style="margin-top:0">Priority domains</div>' + flagged.map(function(d){ var di = ST.d.indexOf(d); return '<button type="button" class="tag tr" style="cursor:pointer;border:none" onclick="IAM.gd(' + di + ')">' + esc(DOM[di] ? DOM[di].name : "") + '</button>'; }).join("") : "") +
    ...
    '<label for="py-overview">Clinical overview</label><textarea id="py-overview" oninput="IAM.setPsych(\'overview\',this.value)" placeholder="Summarise your clinical impression.">' + esc(py.overview) + '</textarea>' +
    '<label for="py-goals">Interview goals</label><textarea id="py-goals" oninput="IAM.setPsych(\'goals\',this.value)" placeholder="What do you want to explore?">' + esc(py.goals) + '</textarea>' +
    '<label for="py-notes">Pre-interview clinical notes</label><textarea id="py-notes" oninput="IAM.setPsych(\'notes\',this.value)" placeholder="Diagnoses, history, context.">' + esc(py.notes) + '</textarea>' +
    '<label for="py-readiness">Participant readiness</label><textarea id="py-readiness" oninput="IAM.setPsych(\'readiness\',this.value)" placeholder="Fatigue tolerance, communication needs, best time of day.">' + esc(py.readiness) + '</textarea></div>' +
